...
private:
	// Скорость с которой игрок перемещается по экрану
	float speed = 0.45;

	// Время задержки между выстрелами
	float shotDelay = 300; // в миллисекундах
	// Часы отвечающие за подсчёт времени прошедшего с момента последнего выстрела
	Clock counterShots;
...
public:
	// создаём лист, который будет хранить пули, выпущенные на поле (по аналогии с enemies)
	...
	void control(float time) {
		...
		{
			// Если подбит - использовать метод  для взрыва
			explosion(time);
		}
	}
	...
	void Shot() {
		...
		bullets.push_back(new Bullet(IntRect(121, 0, 9, 42)));
