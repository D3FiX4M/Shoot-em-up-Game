...
private:
	...
	// спрайт данного объекта
	Sprite sprite;

	// Массив точек для сложных спрайтов (описывается с левой верхней точки)
	// он же контур объекта
	VertexArray outline; 

	// Ширина и высота текущего объекта
	float width;
	float height;

	// Логическая переменная определяющая жив объект или нет
	bool life = true;

	// Текущий кадр взырва
	float currentFrameExplosion = 0;
	// скорость анимации взрыва
...
public:
	// Конструктор класса
	Entity(IntRect rect) {
		// задаём спрайту текстуру и его позицию на ней
